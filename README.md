...
The plan I wrote in MG1 break-down activity connects to the code I wrote because I had the attributes and actions of the player, plant, and UI laid out which helped me write what code was 
needed for each GameObject. I wrote down that the attributes of the player was that it had number of plant seeds so in the Start method I wrote the code _numSeedsLeft = _numSeeds; with _numSeeds already 
...
float movementY = Input.GetAxisRaw("Vertical"); UnityEngine.Vector2 movement = new UnityEngine.Vector2(movementX, movementY); so that if they player presses the WASD keys, they will be able to move. 
...
In my plan, I wrote that the UI's attribute was that it has text that showed the number of seeds remaining and the number of seeds planted. The actions I wrote down is that when the player plants 
...
_plantedText.text = "Seeds Planted: " + seedsPlanted; to ensure that both texts is updated everytime a seed is planted. In the PlantSeed Method, I wrote the code _plantCountUI.UpdateSeeds(_numSeedsLeft, _numSeedsPlanted);
